#--------------------------------------
# ver4 _ 조금 더 정확한 데시벨 측정 및 이동평균 (새로운 마이크)
#---------------------------------------
import os
...
# 마이크와 ADC 설정
mic_sensitivity = 31.62e-3  # mV/Pa, 마이크 사양에 맞게 조정
adc_max_voltage = 1.0       # V, ADC 최대 입력 전압
adc_bits = 16
max_int = 2**(adc_bits - 1) - 1
reference_pressure = 20e-6 # 기준 압력 (Pascal)
...
# 데시벨 변환
def rms_to_db(rms_pressure, reference_pressure):
    if rms_pressure <= 0:
        # rms_pressure가 음수 또는 0인 경우, 기존 측정되고 있는 값에 add_value를 더함
        return -np.inf
    db_value = 10 * np.log10(rms_pressure / reference_pressure)
    return db_value
...
def record_audio():
    ...
    for i in range(0, int(RATE / CHUNK * RECORD_SECONDS)):  # RECORD_SECONDS = 10 <<------------변경필요!!----------------
        data = stream.read(CHUNK)
        rms_pressure = calculate_rms(data) # rms 값 계산
        db_value = rms_to_db(rms_pressure, reference_pressure) # 데시벨 값 계산
        frames.append(data)
        ...
        point = Point("raw_data").tag("location", "home").field("db_value", db_value).time(current_timestamp, WritePrecision.NS)
    ...
    # # InfluxDB에서 데이터 조회 및 그래프 생성 ver.1
    ...
    # InfluxDB에서 데이터 조회 및 그래프 생성 ver.2
    query_api = client.query_api()
    query = f'from(bucket: "{bucket}") |> range(start: -10s) |> filter(fn: (r) => r._measurement == "raw_data")'
    result = query_api.query(query)
    times = []
